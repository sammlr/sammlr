    ...
    if trophy:
        ...
        <div class="trophy-popup-overlay">
            <div class="trophy-popup">
                ...
                <p>
                    {"<strong>Neuer Patch freigeschaltet</strong><br>" + trophy if "," not in trophy else f"<strong>{count} neue Patches erhalten</strong><br>{trophy.replace(',', '<br>')}"}
                </p>
...
def add(album_id, code):
    _, _, gesammelt_vorher, _, _, total = lade_album(album_id)
    _, _, trophies = trophy_status(album_id, gesammelt_vorher, total)
    vorher_erreicht = [titel for ziel, titel in trophies if gesammelt_vorher >= ziel]
    if album_id == "em24":
        _, by_code_vorher, _, _, _, _ = lade_album(album_id)

    for titel, beschreibung, erreicht in em24_spezial_trophaeen(by_code_vorher):
        if erreicht:
            vorher_erreicht.append(titel)

    con = get_db()
    cur = con.cursor()

    daten = con.execute(
        "SELECT * FROM stickers WHERE album_id=? AND sticker_code=?",
        (album_id, code)
    ).fetchone()
    ...
    _, _, gesammelt_nachher, _, _, total = lade_album(album_id)
    _, _, trophies = trophy_status(album_id, gesammelt_nachher, total)
    nachher_erreicht = [titel for ziel, titel in trophies if gesammelt_nachher >= ziel]
    if album_id == "em24":
        _, by_code_nachher, _, _, _, _ = lade_album(album_id)
    
        for titel, beschreibung, erreicht in em24_spezial_trophaeen(by_code_nachher):
            if erreicht:
                nachher_erreicht.append(titel)
    ...
    if neue_trophies:
        url += f"&trophy={quote(neue_trophies[0])}&trigger={quote(code)}"
...
def album_trophaeen(album_id):
    ...
    if album_id == "em24":
        html += "<h2>Gruppen-Trophäen</h2>"

        for gruppe, titel, erreicht in em24_gruppen_trophaeen(by_code):
            klasse = "trophy-unlocked" if erreicht else "trophy-locked"
            beschreibung = f"Alle Sticker aus {gruppe} gesammelt"
            ...
            """
        html += "<h2>Spezial-Patches</h2>"

        for titel, beschreibung, erreicht in em24_spezial_trophaeen(by_code):
            klasse = "trophy-unlocked" if erreicht else "trophy-locked"
