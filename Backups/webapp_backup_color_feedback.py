...
def filter_ok(filter_name, code, by_code):
    ...
    if filter_name == "duplicate":
        return q >= 2
    return True
def filter_count(filter_name, album_id, by_code):
    codes = all_codes(album_id)

    if filter_name == "missing":
        return len([c for c in codes if c not in by_code or by_code[c]["quantity"] == 0])

    if filter_name == "owned":
        return len([c for c in codes if c in by_code and by_code[c]["quantity"] == 1])

    if filter_name == "duplicate":
        return len([c for c in codes if c in by_code and by_code[c]["quantity"] >= 2])
...
def albumseite(album_id):
    ...
    <p>
    Angezeigt:
    <strong>{filter_count(filter_name, album_id, by_code)}</strong>
    Sticker
    </p>
    """
