...
def lade_album(album_id):
    ...
    return album, by_code, gesammelt, doppelte, prozent
...
def startseite():
    ...
    for a in alben:
        album, by_code, gesammelt, doppelte, prozent = lade_album(a["id"])
...
def albumseite(album_id):
    ...
    album, by_code, gesammelt, doppelte, prozent = lade_album(album_id)
...
def sticker_detail(album_id, code):
    ...
    <div class="card">
        ...
        <div class="slot {farbe}" style="width:130px;font-size:26px;margin-bottom:20px;">{display_code(code)}</div>
        <h2>{status}</h2>
        <p>Anzahl: {q}</p>
...
def album_trophaeen(album_id):
    ...
    <div class="trophy"><h2>🏆 Album vollendet</h2><p>Diese Auszeichnungen bauen wir später aus.</p></div>
    </div></body></html>
    """
